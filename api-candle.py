...
def on_open(ws):
    ...
    '''
    products = ["BTC-USD", "ETH-USD", "SOL-USD", "XRP-USD", "DOGE-USD", "ADA-USD", "AVAX-USD", "TON-USD", "SHIB-USD"]    
    print(products)
    # candles ors level2 
    subscribe_to_products(ws, products, CHANNEL_NAMES["candles"])
    ...
    try:
        while True:
            if (datetime.utcnow() - start_time).total_seconds() > 5 and not sent_unsub:
                ...
                unsubscribe_to_products(ws, ["BTC-USD"], CHANNEL_NAMES["candles"])
                ws.close()
                sent_unsub = True
            time.sleep(1)
